...
var car2 = {
    ...
    page_translate: function (node) {
        ...
        scale = 1 - Math.abs(y * 0.3 / $(window).height());
        y_2 = y_2 / 5;
    ...
    haddle_envent_fn: function () {
        ...
        car2._on('success', function () {
            ...
            if (car2._page.eq(car2._pageNext).next().length != 0) {
            ...
            setTimeout(function () {
                ...
                if (car2._pageNext == car2._pageNum - 1) {
                    car2._arrowNode.addClass('f-hide');
                } else {
                    car2._arrowNode.removeClass('f-hide');
                }
               ...
               // $('.txt-arrow').removeClass('z-toggle');
                ...
                if (car2._page.eq(car2._pageNow).next().length == 0) {
    ...
    lazy_img: function () {
        ...
        lazyNode.each(function () {
            var self = $(this);
            if (self.is('img')) {
                self.attr('src', 'images/loading_large.gif');
            } else {
                // 把原来的图片预先保存下来
    ...
    lazy_bigP: function () {
        for (var i = 3; i <= 5; i++) {
            var node = car2._page.eq(car2._pageNow + i);
            if (node.length == 0) break;
            if (node.find('.lazy-img').length != 0) {
            ...
            if (self.attr('data-bg') != 'no') {
                $('<img />')
                    .on('load', function () {
                        ...
                        // 判断下面页面进行加载
                        if (goon) {
                            for (var i = 0; i < car2._pageNum; i++) {
                                var page = car2._page.eq(i);
                                if (car2._page.find('.lazy-img').length == 0) {
                                    continue;
                                } else {
    ...
    height_auto: function (ele, val) {
        ...
        if (!vial) {
            if (ele.height() <= height) {
                ...
                if ((!car2._rotateNode.hasClass('fixed')) && val == 'true'){
                    car2._rotateNode.addClass('fixed');
                }
            } else {
                car2._scrollStart();
                if (val == 'true') {
    ...
    start_callback: function () {
        // 隐藏蒙板
        car2._maskNode.removeClass('z-show');

        setTimeout(function () {
            car2._maskNode.addClass('f-hide');
        }, 1500);
    ...
    styleInit: function () {
        ...
        // 模版提示文字显示
        setTimeout(function () {
            car2._alertNode.find('strong').addClass('z-show');
        }, 1000);

        car2._arrowNode.on('touchmove', function (e) {
            e.preventDefault();
        })

        car2._rotateNode.height($(window).height());
        car2._page.height($(window).height());
        car2._maskNode.height($(window).height());
    ...
    init: function () {
        ...
        $(window).on('load', function () {
            ...
            setTimeout(function () {
                ...
                // 模版提示隐藏
                setTimeout(function () {
                    car2._alertNode.addClass('f-hide');
                }, 1000);

                // 显示正面
                car2._maskNode.addClass('z-show');
